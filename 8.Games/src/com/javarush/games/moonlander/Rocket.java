...
public class Rocket extends GameObject {
    ...
    public Rocket(double x, double y) {
        ...
        List<int[][]> fd = new ArrayList();
        fd.add(ShapeMatrix.FIRE_DOWN_1);
        fd.add(ShapeMatrix.FIRE_DOWN_2);
        fd.add(ShapeMatrix.FIRE_DOWN_3);
    ...
    private void switchFire(boolean isUpPressed, boolean isLeftPressed, boolean isRightPressed) {
        ...
        if (isLeftPressed) {
            leftFire.x = x+matrix[0].length;
            leftFire.y = y+matrix.length;
            leftFire.show();
        }
        else leftFire.hide();
